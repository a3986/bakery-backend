...
app.get('/api/orders', async (req, res) => {
  ...
  try {
    const { status, relativeDays, startDate, endDate } = req.query;
    let query = `
      SELECT o.*, 
      json_agg(json_build_object('name', oi.product_name, 'quantity', oi.quantity, 'total', oi.total_price)) as items 
      FROM orders o 
    ...
    if (relativeDays) {
      conditions.push(`o.created_at >= NOW() - INTERVAL '1 day' * $${paramCount++}`);
      values.push(relativeDays);
...
app.patch('/api/orders/:id/status', async (req, res) => {
  ...
  try {
    ...
    const emailSubject = `Order #${id} Update: ${status.toUpperCase()}`;
    const emailHtml = `
      <h2>Order Update</h2>
      <p>Hi ${order.customer_name},</p>
      <p>Your order #${id} has been <strong>${status}</strong>.</p>
      ${message ? `<p><strong>Message from Bakery:</strong> ${message}</p>` : ''}
      <hr/>
      <p>Thank you for choosing Crumbs A Microbakery!</p>
    `;
